  ...
  methods: {
    async getCustomersByProgramId(ProgramId) {
      try {
        const res = await fetch(
          "http://localhost:8081/customers?programid=" + ProgramId
        );
    ...
    async saveNewSignature() {
      ...
      const response = await fetch(
        "http://localhost:8081/newsignature",
        requestOptions
      );
    ...
    <div v-if="showCustomerListView">
      ...
      <ul class="nav nav-tabs nav-justified">
        <li class="nav-item">
          <a
            class="nav-link"
            v-on:click.prevent="setActive('kids')"
            :class="{ active: isActive('kids') }"
            v-on:click="getCustomersByProgramId(4)"
            >Kids</a
          >
        </li>
        <li class="nav-item">
          <a
            class="nav-link"
            v-on:click.prevent="setActive('juniors')"
            :class="{ active: isActive('juniors') }"
            v-on:click="getCustomersByProgramId(5)"
            >Juniors</a
          >
        </li>
        <li class="nav-item">
          <a
            class="nav-link"
            v-on:click.prevent="setActive('adults')"
            :class="{ active: isActive('adults') }"
            v-on:click="getCustomersByProgramId(6)"
            >Adults</a
          >
        </li>
      </ul>
      <div class="tab-content py-3" id="myTabContent">
        <div
          class="tab-pane fade"
          :class="{ 'active show': isActive('kids') }"
          id="kids"
        >
          Kids
        </div>
        <div
          class="tab-pane fade"
          :class="{ 'active show': isActive('juniors') }"
          id="juniors"
        >
          Juniors
        </div>
        <div
          class="tab-pane fade"
          :class="{ 'active show': isActive('adults') }"
          id="adults"
        >
          Adults
        </div>
